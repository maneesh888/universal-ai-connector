...
| P8 | High, phased host and distribution work | Deliver sample-host live workflows sequentially for iOS, Android, Kotlin/JVM console, and Compose desktop, prove parity, then add publication, released-artifact consumers, signing, and checksums without duplicating connector behavior per host |
...
| P8 | Production distribution and host integration | In progress | P8-A iOS is accepted through PR #69; P8-B Android closeout is recorded in PR #75; P8-C source hosts are merged through PR #76; bounded Windows deterministic/source-launch proof, exact-head OpenAI, Anthropic, and OpenRouter console live proof, and hosted OpenAI-compatible Gateway adapter proof are recorded; the separately configured loopback Gateway deployment, interactive desktop live proof, and multi-host acceptance remain outstanding; parity and distribution packages remain not started |
...
Gateway-specific adapter or host surface. P7-C reconciles Gateway-representative concurrency,
close races, cleanup, redaction, copy-paste Kotlin and Swift configuration, and the existing host
package boundaries without activating P8.
