...
The read-only reviewer must report a blocking `milestone closeout missing or inconsistent` finding when a `completes` brief lacks the required transition, repository status documents disagree, more than one milestone would be `In progress`, the next milestone is activated prematurely, or the active plan's acceptance criteria are not satisfied. The reviewer never edits closeout files.
