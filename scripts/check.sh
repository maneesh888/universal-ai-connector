...
verify_public_artifact_signatures() {
  ...
  if grep -Eq \
    'com\.myadidi\.universalai\.connector\.internal\.provider\.(openai|anthropic|openrouter|openaicompatible)\.|(OpenAi|Anthropic|OpenRouter)[A-Za-z0-9_]*(Wire|Adapter|Translator)' \
    "$signatures"; then
