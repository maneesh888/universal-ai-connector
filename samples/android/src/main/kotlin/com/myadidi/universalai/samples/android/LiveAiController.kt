...
class LiveAiController(
    ...
    fun saveCredential(value: String) {
        ...
        try {
            ...
            resetInteraction()
            mutableState.value = state.value.copy(credentialStored = true, status = "Credential protected by Android Keystore.")
        } catch (_: Exception) {
    ...
    fun importSeed(seed: LiveProofSeed): Boolean {
        if (closed) return false
        configure(seed.configuration.provider, seed.configuration.baseUrl)
        saveCredential(seed.credential)
        if (!state.value.credentialStored) return false
        preferredModel = seed.model
        mutableState.value = state.value.copy(status = "Development credential imported into protected storage.")
        return true
    }
    ...
    fun clearConfiguration() {
        ...
        try {
            store.clearAll()
            mutableState.value = LiveUiState(status = "Live configuration and protected credentials cleared.")
        } catch (_: Exception) {
    ...
    private fun applyModels(result: LiveModels) {
        mutableState.value = when (result) {
            is LiveModels.Supported -> state.value.copy(
                ...
                selectedModel = (state.value.selectedModel ?: preferredModel)?.takeIf { it in result.ids },
            ...
            LiveModels.Unsupported -> state.value.copy(
                ...
                manualModel = state.value.manualModel.ifEmpty { preferredModel.orEmpty() },
    ...
    private fun refreshCredential() {
        try {
            ...
            mutableState.value = state.value.copy(credentialStored = stored, status = if (stored) "Protected credential available." else "No live credential saved.")
        } catch (_: Exception) {
