...
class LiveAiControllerTest {
    ...
    @Test fun clearCancelsDiscoveryRemovesAllCredentialsAndSeedPreference() = runTest {
        ...
        val controller = LiveAiController(this, store, LiveClientFactory { _, _ -> client })
        assertTrue(controller.importSeed(seed("model-b")))
    ...
    @Test fun seedSelectsOnlyExactDiscoveredIdentity() = runTest {
        val client = Client()
        val controller = LiveAiController(this, Store(), LiveClientFactory { _, _ -> client })
        controller.importSeed(seed("model-b")); controller.loadModels(); advanceUntilIdle()
        assertEquals("model-b", controller.state.value.selectedModel)
        controller.importSeed(seed("model-B")); controller.loadModels(); advanceUntilIdle()
        assertNull(controller.state.value.selectedModel)
        assertFalse(controller.state.value.canTest)
    }
    ...
    private fun seed(model: String): LiveProofSeed = LiveProofSeed.read(ByteArrayInputStream(frame(model = model)), true, 2000)
}

internal fun frame(provider: String = "openai", base: String = "", model: String = "vendor/exact:version", credential: String = "synthetic-credential"): ByteArray =
    ByteArrayOutputStream().also { output ->
        DataOutputStream(output).use { data ->
            data.writeInt(1)
            listOf(provider, base, model, credential).forEach { value ->
