...
- After creating or updating a draft pull request and its review brief, the same root agent must immediately invoke the project `pr-reviewer` custom agent for an independent read-only review of the exact head SHA. Do not wait for GitHub Actions to finish; review and required checks run in parallel. The root agent remains active and responsible for verification and every GitHub state change.
...
- Mark a draft ready only after the review-completion gate passes and the current user request explicitly authorizes that state change. A single request may authorize creating or updating the draft, running review, fixing in-scope findings, marking the clean reviewed head ready, and enabling auto-merge; when that authorization is explicit, continue the same active task without asking for a second confirmation. When the current user request explicitly authorizes merging and that gate passes, refresh the head, required checks, review threads, requested changes, and mergeability; mark the draft ready; refresh those states again immediately; then enable GitHub native auto-merge with squash and exact-head protection:
